<template>
    <div>
        <div class="text-xl text-snhm mb-2">
            <span>Skabelonoversigt: {{ formularStore.licenseeName }}</span>
        </div>
        <div class="grid grid-cols-[max-content_max-content_max-content_max-content_1fr] gap-x-3">
            <template v-for="(skabelon, index) in formularStore.getTemplateModelList()">
                ...
                <div v-if="index == 0" class="font-semibold border-e pe-2 border-gray-300">
                    <span v-if="!formularStore.isAdministrator">Handlinger</span>
                </div>
                ...
                <div class="border-e border-gray-300 pe-2">
                    <router-link :to="`/skabelonnavn/kopier/${skabelon.formTemplateNameType.id}`"
                        v-if="!formularStore.isAdministrator && skabelon.formTemplateNameType.licenseeId == undefined">
                        Kopier
                    </router-link>
                </div>
...
function getScenarieNavnById(id: number): string {
    return formularStore.bandingScenarioList.find((item) => item.id == id)?.name ?? 'ukendt';
}

function canEdit(licenshaverId: number): boolean {
    return formularStore.isAdministrator || licenshaverId == formularStore.LicenseeId;
}
</script>